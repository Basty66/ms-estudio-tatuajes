...
export async function POST(request: Request) {
  try {
    ...
    if (sizeInBytes > MAX_SIZE) {
      return Response.json(
        { success: false, error: "La imagen no puede superar 200 KB" },
        { status: 400 },
      )
    }
    ...
    // Subir a Vercel Blob
    const filename = `carnets/${Date.now()}-${Math.random().toString(36).slice(2, 8)}.${ext}`
    ...
    console.error("upload-carnet error:", String(error))
    return Response.json(
